...
/* harmony import */ var fullpage_js_dist_fullpage_min_css__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(6);
...
var Fullpage=void 0;// NOTE: SSR detection, will bail on no window
try{var w=window;// eslint-disable-line no-unused-vars
Fullpage=__webpack_require__(1);// eslint-disable-line global-require
...
var newArgs=[key].concat(args);_this2.update.apply(_this2,_toConsumableArray(newArgs));};return result;},{});var finalOpts=_extends({},this.props,listeners);console.log(finalOpts);if(v2compatible){if(!$||$ instanceof window.jQuery===false){throw new Error('Must provide $ (jQuery) as a prop if using v2 API');}$(document).ready(function(){// eslint-disable-line
$('#fullpage').fullpage(finalOpts);});}else if(Fullpage){new Fullpage('#fullpage',finalOpts);// eslint-disable-line
this.markInitialized();}}},{key:'componentWillUnmount',value:function componentWillUnmount(){// NOTE: need to check for init to support SSR
if(!this.state.initialized)return;this.fullpageApi.destroy('all');}},{key:'markInitialized',value:function markInitialized(){this.fullpageApi=window.fullpage_api;this.setState({initialized:true});}},{key:'update',value:function update(lastEvent){for(var _len2=arguments.length,args=Array(_len2>1?_len2-1:0),_key2=1;_key2<_len2;_key2++){args[_key2-1]=arguments[_key2];}var _props$v2compatible2=this.props.v2compatible,v2compatible=_props$v2compatible2===undefined?false:_props$v2compatible2;var state=_extends({},this.state);var makeState=function makeState(callbackParameters){return _extends({},state,callbackParameters,{callback:lastEvent});};var fromArgs=function fromArgs(argList){return argList.reduce(function(result,key,i){var value=args[i];result[key]=value;// eslint-disable-line
